...
{
    private static readonly string UberShapeEncoded =
        "%7B%22%24cache%22%3A3%2C%22users%22%3A%5B%7B%22id%22%3A0%2C%22name%22%3A%22string%22%2C%22email%22%3A%22string%22%2C%22isActive%22%3Atrue%7D%5D%2C%22meta%22%3A%7B%22total%22%3A0%2C%22next%22%3A%22string%22%7D%7D";

    private static readonly string UberShapeSanitized =
        "{\"users\":[{\"id\":0,\"name\":\"string\",\"email\":\"string\",\"isActive\":true}],\"meta\":{\"total\":0,\"next\":\"string\"}}";

    private readonly CustomFactory _factory;

    public HttpSamplesSmokeTests(CustomFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task Uber_Get_Includes_Schema_Header_And_Returns_Json()
    {
        ...
        using var req = new HttpRequestMessage(HttpMethod.Get,
            $"/api/mock/users?limit=2&offset=0&includeSchema=true&shape={UberShapeEncoded}");
    ...
    public async Task Header_Shape_Get_Works_And_Echoes_Schema()
    {
        ...
        req.Headers.Accept.Clear();
        req.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
        req.Headers.TryAddWithoutValidation("X-Response-Shape",
            "{\"products\":[{\"sku\":\"string\",\"price\":0.0,\"tags\":[\"string\"]}],\"count\":0}");

        var res = await client.SendAsync(req);
        Assert.Equal(HttpStatusCode.OK, res.StatusCode);
        Assert.True(res.Headers.TryGetValues("X-Response-Schema", out var values));
        Assert.Equal("{\"products\":[{\"sku\":\"string\",\"price\":0.0,\"tags\":[\"string\"]}],\"count\":0}",
            values!.First());
    }
    ...
    public async Task Post_Order_Succeeds()
    {
        ...
        req.Headers.Accept.Clear();
        req.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
        req.Content = new StringContent("{\"customerId\":\"cus_123\",\"items\":[{\"sku\":\"ABC-001\",\"qty\":2}]}",
        ...
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureServices(services =>
            {
                ...
                services.AddHttpClient("LLMockApi")
                    .ConfigurePrimaryHttpMessageHandler(() => new FakeCompletionsHandler());
    ...
    private class FakeCompletionsHandler : HttpMessageHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var payload =
                "{\"choices\":[{\"message\":{\"content\":\"{\\\"ok\\\":true,\\\"source\\\":\\\"fake-llm\\\"}\"}}]}";
