...
public class MockEndpointConfig
{
    ...
    internal void ApplyToRequest(HttpRequestMessage request)
    {
        ...
        if (Backend != null)
        {
            request.Headers.TryAddWithoutValidation("X-LLM-Backend", Backend);
        }
        ...
        if (Error != null)
        {
            ...
            {
                request.Headers.TryAddWithoutValidation("X-Error-Message", Error.Message);
            }
            if (Error.Details != null)
            {
                request.Headers.TryAddWithoutValidation("X-Error-Details", Error.Details);
            }
        }
        ...
        {
            queryParams.Add($"cache={CacheSize.Value}");
        }

        if (Backend != null)
        {
            queryParams.Add($"backend={Uri.EscapeDataString(Backend)}");
        }

        if (SseMode != null)
        {
            queryParams.Add($"sseMode={Uri.EscapeDataString(SseMode)}");
        }

        if (Continuous.HasValue)
        {
            queryParams.Add($"continuous={Continuous.Value.ToString().ToLower()}");
        }

        if (ContinuousInterval.HasValue)
        {
            queryParams.Add($"interval={ContinuousInterval.Value}");
        }

        if (AutoChunk.HasValue)
        {
            queryParams.Add($"autoChunk={AutoChunk.Value.ToString().ToLower()}");
        }
