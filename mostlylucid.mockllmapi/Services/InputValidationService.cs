...
public class InputValidationService : IInputValidationService
{
    ...
    {
        if (string.IsNullOrWhiteSpace(shape)) return new ValidationResult(true); // Empty shape is valid

        // Check length
        if (shape.Length > 5000) return new ValidationResult(false, "Shape too long (max 5000 characters)");

        // Check for dangerous patterns
        var dangerousCheck = CheckForDangerousContent(shape);
        if (!dangerousCheck.IsValid) return dangerousCheck;
    ...
    {
        if (string.IsNullOrWhiteSpace(journeyName)) return new ValidationResult(false, "Journey name is required");

        if (journeyName.Length > 100) return new ValidationResult(false, "Journey name too long (max 100 characters)");

        if (!ValidationRegex.SafeNameRegex().IsMatch(journeyName))
            return new ValidationResult(false,
                "Journey name contains invalid characters. Use only letters, numbers, underscores, and hyphens");
    ...
    {
        if (string.IsNullOrWhiteSpace(toolName)) return new ValidationResult(false, "Tool name is required");

        if (toolName.Length > 50) return new ValidationResult(false, "Tool name too long (max 50 characters)");

        if (!ValidationRegex.SafeNameRegex().IsMatch(toolName))
            return new ValidationResult(false,
                "Tool name contains invalid characters. Use only letters, numbers, underscores, and hyphens");
    ...
    {
        if (string.IsNullOrWhiteSpace(json)) return new ValidationResult(false, "JSON input is required");

        if (json.Length > 10000) return new ValidationResult(false, "JSON too long (max 10000 characters)");
    ...
    {
        if (string.IsNullOrWhiteSpace(endpoint)) return new ValidationResult(false, "Endpoint is required");

        if (endpoint.Length > 500) return new ValidationResult(false, "Endpoint too long (max 500 characters)");
        ...
        if (Uri.TryCreate(endpoint, UriKind.RelativeOrAbsolute, out var uri))
        {
            ...
            if (uri.IsAbsoluteUri)
            {
                ...
                if (!allowedSchemes.Contains(uri.Scheme.ToLowerInvariant()))
                    return new ValidationResult(false,
                        $"Invalid URI scheme: {uri.Scheme}. Only HTTP and HTTPS are allowed");
    ...
    public ValidationResult ValidateParameter(string name, object? value)
    {
        ...
        if (string.IsNullOrWhiteSpace(name)) return new ValidationResult(false, "Parameter name is required");

        if (name.Length > 50) return new ValidationResult(false, "Parameter name too long (max 50 characters)");
        ...
        if (value != null)
        {
            ...
            if (!dangerousCheck.IsValid)
                return new ValidationResult(false,
                    $"Parameter '{name}' contains dangerous content: {dangerousCheck.ErrorMessage}");
        }
    ...
    public string SanitizeString(string input, int maxLength = 1000)
    {
        ...
        var sanitized = input;
        foreach (var regex in ValidationRegex.DangerousPatterns) sanitized = regex.Replace(sanitized, "");
    ...
    public string SanitizeForPrompt(string input, int maxLength = 2000)
    {
        ...
        foreach (var regex in ValidationRegex.PromptInjectionPatterns)
            ...
            {
                _logger.LogWarning("Potential prompt injection attempt detected and neutralized: {Pattern}",
                    regex.ToString());
    ...
    {
        if (string.IsNullOrEmpty(input)) return new ValidationResult(true);
        ...
        foreach (var regex in ValidationRegex.PromptInjectionPatterns)
            if (regex.IsMatch(input))
            {
                ...
                return new ValidationResult(false,
                    $"Input contains potential prompt injection attempt ({patternName})");
            }
