...
public class AuthMiddleware
{
    ...
    private string? ExtractToken(HttpRequest request)
    {
        ...
        if (request.Query.TryGetValue("token", out var tokenParam) && tokenParam.Count > 0)
        {
            return tokenParam[0];
        }
    ...
    private TokenValidationResult ValidateToken(string token)
    {
        ...
        {
            return ValidateJwtToken(token, secret);
        }
    ...
    private TokenValidationResult ValidateJwtToken(string token, string secret)
    {
        try
        {
            ...
            if (parts.Length != 3)
            {
                return TokenValidationResult.Failed("Invalid JWT format");
            }
            ...
            if (payloadRoot.TryGetProperty("exp", out var expElement))
            {
                ...
                if (now > expTime.Add(ClockSkewTolerance))
                {
    ...
    private TokenValidationResult ValidateApiKey(string apiKey, string secret)
    {
        ...
        if (!CryptographicEquals(apiKey, secret))
        {
            return TokenValidationResult.Failed("Invalid API key");
        }
    ...
    private async Task HandleUnauthorized(HttpContext context, string message)
    {
        ...
        var errorResponse = new
        {
            ...
            hint = "Provide a valid Bearer token via Authorization header, or set ManagementAuthSecret and use it as X-API-Key header"
        };
    ...
    private class TokenValidationResult
    {
        ...
        public static TokenValidationResult Failed(string reason) =>
            new() { IsValid = false, ErrorReason = reason };
    }
}
