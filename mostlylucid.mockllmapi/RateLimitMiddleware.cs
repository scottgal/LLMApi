...
public class RateLimitMiddleware : IDisposable
{
            ...
            if (requestSizeLimit > 0 && context.Request.ContentLength > requestSizeLimit)
            {
                ...
                var errorMessage =
                    SanitizeErrorMessage(
                        $"Request size exceeds maximum allowed size of {requestSizeLimit / (1024 * 1024)}MB");
    ...
    private string GetClientIdentifier(HttpContext context)
    {
        ...
        if (!string.IsNullOrWhiteSpace(authHeader)) return $"auth:{ComputeHash(authHeader)}";
        ...
        if (!string.IsNullOrWhiteSpace(forwardedFor))
        {
            ...
            if (!string.IsNullOrWhiteSpace(clientIp)) return $"ip:{clientIp}";
        }
    ...
    private void CleanupStaleClients(object? state)
    {
        ...
        foreach (var client in staleClients) _clientLastSeen.TryRemove(client, out _);
